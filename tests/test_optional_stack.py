...
def test_mlflow_adapter_reports_missing_dependency(tmp_path):
    ...
    bundle.mkdir()
    (bundle / 'report.json').write_text(json.dumps({
        'run_id': 'abc', 'source': {'kind': 'synthetic'}, 'config': {},
        'test_metrics': {'arrival_learned': {'mae': 1.0, 'rmse': 2.0, 'mean_wait_seconds': 3.0}}
    }), encoding='utf-8')
    try:
...
def test_mlflow_metric_selection_follows_report_policy_forecaster():
    from asofcast.mlops import selected_policy_metrics
    report = {
        'policy_selection': {'forecaster': 'calibrated'},
        'test_metrics': {
            'arrival_learned': {'mae': 9.0, 'rmse': 9.0, 'mean_wait_seconds': 0.0},
            'calibrated_learned': {'mae': 1.0, 'rmse': 2.0, 'mean_wait_seconds': 3.0},
        },
    }
    name, metrics = selected_policy_metrics(report)
    assert name == 'calibrated_learned'
